...
class iperf3_plotter(object):
    ...
    def plotLine(self, dataset, filename, desc, title, figureN):
        ...
        for c in dataset.columns:
            ...
            if self.upperbond > 0: axs[i].plot(dataset.index, [upperbond] * len(dataset.index), "--", lw=2, color="red",
                                               alpha=0.5)
            if self.upperbond > 0: axs[i].plot(dataset.index, [lowerbond] * len(dataset.index), "--", lw=2, color="red",
                                               alpha=0.5)
    ...
    def plotLinePretty(self, dataset, filename, desc, title):
        ...
        plt.yticks(np.arange(y_min, y_max, y_level), [str(round(x, 3)) for x in np.arange(y_min, y_max, y_level)],
                   fontsize=14)
        ...
        for (upper_bound, lower_bound, tags) in self.bound:
            ...
            if upper_bound > 0: plt.plot(dataset.index, [upper_bound] * len(dataset.index), lw=2,
                                         color=tableau20[j % 20])
            plt.text(x_max, upper_bound, tags, fontsize=12, color=tableau20[j % 20])
            if lower_bound > 0: plt.plot(dataset.index, [lower_bound] * len(dataset.index), lw=2,
                                         color=tableau20[j % 20])
...
class iperf3_dataParser(object):
    ...
    def parseOptions(self, args):
        ...
        if options.plotFiles == "":
            ...
            self.plotFiles = map(lambda x: x.strip(), options.plotFiles.split(",")) if "," in options.plotFiles else [
                options.plotFiles.strip()]

        if options.noPlotFiles == "":
            self.noPlotFiles = []
        else:
            self.noPlotFiles = map(lambda x: x.strip(),
                                   options.noPlotFiles.split(",")) if "," in options.noPlotFiles else [
                options.noPlotFiles.strip()]
        ...
        if options.bound == "":
            ...
            # print(options.bound)
            bounds_tmp = map(lambda x: x.strip(), options.bound.split("]")) if "]" in options.bound else [
                options.bound.strip()]
        ...
        return (self.foldername, list(self.plotFiles), list(self.noPlotFiles), self.output, options.upperLimit,
                options.lowerLimit, self.bound)
...
if __name__ == '__main__':
    ...
    (foldername, plotFiles, noPlotFiles, output, upperLimit, lowerLimit, bound) = s.parseOptions(sys.argv[1:])
    print('foldername {}\nplotFiles {}\nnoPlotFiles {}\noutput {}\nupperLimit {}\nlowerLimit {}\nbound {}\n'.format(
        foldername, str(plotFiles), str(noPlotFiles), output, upperLimit, lowerLimit, str(bound)))
